...
console.log('Order Saved to Store:', newOrder);
return newOrder;
},
...
gridContainer.innerHTML = `
    ...
    <p class="text-xs text-zinc-500">No items match your search criteria.</p>
    </div>
`;
return;
}
...
return `
    <div class="bg-white rounded-xl border border-[#E8E1D5] overflow-hidden shadow-2xs hover:shadow-md transition-shadow flex flex-col justify-between p-4 space-y-3">
    <div class="space-y-3">
        <div class="aspect-[4/5] rounded-lg overflow-hidden bg-zinc-100 border border-[#E8E1D5] relative">
        <img src="${product.images[0]}" alt="${product.name}" class="w-full h-full object-cover transform hover:scale-103 transition-transform duration-500" />
        <span class="absolute top-2.5 left-2.5 px-2.5 py-0.5 rounded-full bg-white/90 backdrop-blur-xs text-[10px] font-bold text-[#5C1D24] border border-[#E8E1D5]">
            ${product.category}
        </span>
        </div>

        <div>
        <h3 class="font-serif text-base font-semibold text-[#1E1E1E]">${product.name}</h3>
        ...
        <div class="flex items-center gap-1.5">
            ...
            ></button>
            `).join('')}
            <span class="text-[11px] text-zinc-600 font-medium ml-1">${selectedColor.name}</span>
        </div>
        </div>
    </div>
    ...
    <div class="pt-2 border-t border-[#F2EDE4] flex items-center justify-between">
        <div>
        <span class="text-[9px] uppercase tracking-wider text-zinc-400 block">Price</span>
        <p class="font-serif text-base font-bold text-[#5C1D24]">₹${product.price.toLocaleString()}</p>
        </div>
        <button
        onclick="addCatalogueToCart('${product.id}')"
        class="px-4 py-2 rounded-full bg-[#5C1D24] hover:bg-[#47151a] text-white text-xs font-semibold flex items-center gap-1.5 shadow-2xs transition-colors cursor-pointer"
        >
...
return `
    <div id="${isCustom ? 'section-custom-hamper' : ''}" class="rounded-xl bg-white border p-5 sm:p-6 flex flex-col justify-between transition-all duration-300 relative shadow-2xs hover:shadow-md ${
    isCustom ? 'border-[#C5A059] ring-1 ring-[#C5A059]/30 bg-gradient-to-br from-white via-[#FAF8F2] to-white' : 'border-[#E8E1D5]'
    }">
    ${isCustom ? `
        <div class="absolute -top-3 left-6 bg-[#C5A059] text-white px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 shadow-xs">
        <i data-lucide="sparkles" class="w-2.5 h-2.5 text-white"></i>
        <span>Interactive Customizer</span>
        </div>
    ` : `
        <div class="absolute -top-3 left-6 bg-[#5C1D24] text-[#FAF7F0] px-2.5 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider shadow-xs">
    ...
    <div class="space-y-4">
        ...
        <div class="sm:col-span-7 space-y-1.5">
            <h3 class="font-serif text-lg sm:text-xl font-medium text-[#1E1E1E] leading-snug">${hamper.name}</h3>
            ...
            <div class="pt-1.5">
            <span class="text-[10px] text-zinc-400 font-light">Price</span>
            <p class="font-serif text-xl font-bold text-[#5C1D24]">₹${hamper.price.toLocaleString()}</p>
            </div>
        </div>
        </div>
        ...
        <div class="pt-3 border-t border-[#F2EDE4] space-y-2">
            <h4 class="text-[11px] font-semibold uppercase tracking-wider text-[#C5A059] flex items-center gap-1.5">
            ...
            ${hamper.hamper_items.map(item => `
                <li class="flex items-start gap-1.5 bg-[#FAF7F0] px-2 py-1 rounded border border-[#EFE8DC] text-[11px]">
...
<div class="bg-white rounded-xl p-5 sm:p-6 border border-[#E8E1D5] shadow-2xs relative flex flex-col justify-between space-y-4 hover:shadow-sm transition-shadow">
    <div class="flex items-center justify-between">
    <span class="w-8 h-8 rounded-full bg-[#5C1D24] text-[#FAF7F0] font-serif font-bold text-sm flex items-center justify-center shadow-xs">
        ${step.step_number}
    </span>
    <div class="w-8 h-8 rounded-full bg-[#FAF7F0] border border-[#C5A059]/40 flex items-center justify-center text-[#C5A059]">
        <i data-lucide="${icons[idx % icons.length]}" class="w-4 h-4"></i>
    </div>
    </div>

    <div class="space-y-1.5">
    <h3 class="font-serif text-base sm:text-lg font-medium text-[#1E1E1E]">${step.title}</h3>
    <p class="text-xs text-zinc-600 font-light leading-relaxed">${step.description}</p>
    </div>

    <div class="pt-2 border-t border-[#F2EDE4] text-[11px] font-medium text-[#C5A059]">
...
<div class="relative rounded-xl overflow-hidden aspect-[3/4] group cursor-pointer border border-[#E8E1D5] shadow-2xs">
    ...
    <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent flex flex-col justify-end p-3 text-white">
    <div class="flex items-center gap-1 text-[10px] text-[#E6CA85] font-semibold mb-0.5">
...
<div class="bg-white p-4.5 rounded-xl border border-[#E8E1D5] shadow-2xs space-y-2 flex flex-col justify-between">
    <div class="space-y-2">
    ...
    <div class="pt-2 border-t border-[#F2EDE4] flex items-center justify-between text-[11px]">
...
<div class="bg-white rounded-lg border border-[#E8E1D5] overflow-hidden transition-all shadow-2xs">
    ...
    <span class="font-serif text-sm sm:text-[15px] font-medium text-[#1E1E1E]">${faq.q}</span>
...
container.innerHTML = cart.map((item, index) => {
    ...
    <div class="p-3 rounded-lg bg-white border border-[#E8E1D5] shadow-2xs flex gap-2.5 relative group">
        <img src="${imgSrc}" alt="${item.product.name}" class="w-16 h-20 rounded-md object-cover bg-zinc-100 flex-shrink-0 border border-[#E8E1D5]" />
        ...
        <div class="space-y-0.5">
            ...
            ${item.selected_color ? `
            <div class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-[#FAF7F0] border border-[#E8E1D5] text-[10px] text-[#5C1D24] font-medium">
                <span>Shade:</span>
            ...
            <div class="text-[10px] bg-[#FAF8F3] p-1.5 rounded border border-[#EFE8DC] space-y-0.5 mt-1">
        ...
        <div class="flex items-center justify-between pt-1.5">
            ...
            <span class="px-2 text-[11px] font-semibold text-zinc-800">${item.quantity}</span>
            <button onclick="updateCartQuantity(${index}, ${item.quantity + 1})" class="px-1.5 py-0.5 text-zinc-600 hover:bg-zinc-200 cursor-pointer">
...
return `
    ...
    <div class="flex items-center gap-2.5">
        <div class="w-5 h-5 rounded-md flex items-center justify-center border transition-colors ${
...
counterEl.className = 'text-sm font-bold px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-800';
} else {
counterEl.className = 'text-sm font-bold px-2.5 py-1 rounded-full bg-amber-100 text-amber-800';
}
...
previewEl.innerHTML = selectedHamperItems.map(name => `
    <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-white border border-[#C5A059]/40 rounded-full text-xs text-zinc-800 font-medium shadow-2xs">
...
<button type="button" onclick="setReviewRating(${star})" class="focus:outline-none cursor-pointer">
    <i data-lucide="star" class="w-6 h-6 ${star <= reviewRating ? 'fill-[#C5A059] text-[#C5A059]' : 'text-zinc-300'}"></i>
</button>
`).join('');
lucide.createIcons();
}
