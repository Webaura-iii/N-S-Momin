...
function renderDashboard() {
  ...
  return `
    <div class="bg-white p-6 rounded-2xl border border-[#EAE3D2] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <span class="text-[10px] font-bold tracking-widest uppercase text-[#C5A059]">Overview &amp; Store Pulse</span>
        <h2 class="font-serif text-3xl font-normal text-zinc-900 mt-1">Admin Workspace</h2>
        <p class="text-xs text-zinc-500 mt-0.5">Real-time synchronization active with Firestore database</p>
      </div>
      <div class="flex items-center gap-2">
        <button onclick="openManualOrderModal()" class="px-4 py-2.5 rounded-xl bg-[#25D366] hover:bg-[#20bd5a] text-white text-xs font-semibold flex items-center gap-2 transition-colors cursor-pointer">
          <i data-lucide="message-square" class="w-4 h-4"></i>
          <span>Log WhatsApp/DM Order</span>
        </button>
        <button onclick="openAddProductModal()" class="px-4 py-2.5 rounded-xl bg-[#5C1D24] hover:bg-[#47161c] text-white text-xs font-semibold flex items-center gap-2 transition-colors cursor-pointer">
    ...
    <!-- Recent Orders Table -->
    <div class="bg-white rounded-2xl border border-[#EAE3D2] p-6 space-y-4">
      <div class="flex items-center justify-between">
        ...
        <button onclick="switchTab('orders')" class="text-xs font-semibold text-[#5C1D24] hover:underline flex items-center gap-1 cursor-pointer">
          ...
          <tbody class="divide-y divide-[#EAE3D2]">
            ${appState.orders.map(order => `
              <tr class="hover:bg-[#FAF7F2]">
                <td class="py-3.5 px-4 font-mono font-bold text-zinc-900">#${order.id}</td>
                ...
                <td class="py-3.5 px-4 text-zinc-600">${order.items}</td>
                <td class="py-3.5 px-4 font-serif font-bold text-zinc-900">₹${order.total.toLocaleString()}</td>
                <td class="py-3.5 px-4">
...
function renderProducts() {
  ...
  return `
    <div class="bg-white p-6 rounded-2xl border border-[#EAE3D2] flex justify-between items-center">
      <div>
        <h2 class="font-serif text-3xl font-normal text-zinc-900">Products &amp; Hampers Catalog</h2>
        <p class="text-xs text-zinc-500 mt-0.5">Manage your hijabs, luxury scarves, and bespoke celebration hamper sets</p>
      </div>
      <button onclick="openAddProductModal()" class="px-4 py-2.5 rounded-xl bg-[#5C1D24] hover:bg-[#47161c] text-white text-xs font-semibold flex items-center gap-2 cursor-pointer transition-colors">
        <i data-lucide="plus" class="w-4 h-4"></i>
    ...
    <!-- Filters Bar -->
    <div class="bg-white p-4 rounded-2xl border border-[#EAE3D2] flex flex-wrap items-center gap-3">
        ...
        <tbody class="divide-y divide-[#EAE3D2]">
          ...
          ` : filteredProducts.map(p => `
            <tr class="hover:bg-[#FAF7F2]">
              ...
              <td class="py-3.5 px-4">
                <div class="flex items-center gap-3">
                  <div class="w-10 h-10 rounded-lg bg-[#FAF7F2] border border-[#EAE3D2] shrink-0 overflow-hidden relative">
                    <img src="https://images.unsplash.com/photo-1584917865442-de89df76afd3?auto=format&fit=crop&w=100&q=80" class="w-full h-full object-cover" />
                  </div>
                  <div>
              ...
              <td class="py-3.5 px-4 text-zinc-600">${p.category}</td>
              <td class="py-3.5 px-4">
              ...
              <td class="py-3.5 px-4 font-serif font-bold text-zinc-900">₹${p.price}</td>
              <td class="py-3.5 px-4 font-medium ${p.stock <= 10 ? 'text-amber-600 font-bold' : ''}">${p.stock}</td>
              <td class="py-3.5 px-4">
...
function renderOrders() {
  ...
  return `
    <div class="bg-white p-6 rounded-2xl border border-[#EAE3D2] flex justify-between items-center">
      <div>
        <h2 class="font-serif text-3xl font-normal text-zinc-900">Orders Management</h2>
      ...
      <button onclick="openManualOrderModal()" class="px-4 py-2.5 rounded-xl bg-[#25D366] hover:bg-[#20bd5a] text-white text-xs font-semibold flex items-center gap-2 cursor-pointer transition-colors">
        <i data-lucide="plus" class="w-4 h-4"></i>
        <span>Manual WhatsApp / DM Order</span>
      </button>
    </div>
...
function renderReviews() {
  return `
    <div class="bg-white p-6 rounded-2xl border border-[#EAE3D2] flex justify-between items-center">
      <div>
        <h2 class="font-serif text-3xl font-normal text-zinc-900">Customer Reviews &amp; Social Proof</h2>
      ...
      <button onclick="openAddReviewModal()" class="px-4 py-2.5 rounded-xl bg-[#5C1D24] hover:bg-[#47161c] text-white text-xs font-semibold flex items-center gap-2 cursor-pointer transition-colors">
        <i data-lucide="plus" class="w-4 h-4"></i>
...
function renderSiteContent() {
  return `
    <div class="bg-white p-6 rounded-2xl border border-[#EAE3D2] flex justify-between items-center">
      <div>
        <h2 class="font-serif text-3xl font-normal text-zinc-900">Site Content &amp; Homepage Editor</h2>
        <p class="text-xs text-zinc-500 mt-0.5">Modify promotional banners, hero imagery, FAQs, and social statistics</p>
      </div>
      <button onclick="alert('Site content successfully published!')" class="px-4 py-2.5 rounded-xl bg-[#5C1D24] hover:bg-[#47161c] text-white text-xs font-semibold flex items-center gap-2 cursor-pointer transition-colors">
    ...
    <!-- Announcement Bar -->
    <div class="bg-white p-6 rounded-2xl border border-[#EAE3D2] space-y-4">
...
function renderSettings() {
  return `
    <div class="bg-white p-6 rounded-2xl border border-[#EAE3D2] space-y-2">
      <h2 class="font-serif text-3xl font-normal text-zinc-900">Settings &amp; Configuration</h2>
    ...
    <div class="bg-white p-6 rounded-2xl border border-[#EAE3D2] space-y-4 max-w-lg">
  ...
  const modalHTML = `
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop">
      <div class="bg-[#FAF7F2] w-full max-w-2xl rounded-2xl border border-[#EAE3D2] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        <div class="px-6 py-4 border-b border-[#EAE3D2] flex justify-between items-center bg-white">
          <h3 class="font-serif text-xl font-normal text-zinc-900">Add New Product or Hamper</h3>
          <button onclick="closeModal()" class="text-zinc-400 hover:text-zinc-700 cursor-pointer"><i data-lucide="x" class="w-5 h-5"></i></button>
        </div>

        <form onsubmit="handleCreateProduct(event)" class="p-6 overflow-y-auto space-y-4">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-semibold text-zinc-700 mb-1">Title / Name *</label>
              <input id="prod-title" type="text" required placeholder="e.g. Royal Sovereign Bridal Chest" class="w-full bg-white" />
            </div>
            <div>
              <label class="block text-xs font-semibold text-zinc-700 mb-1">URL Slug</label>
              <input id="prod-slug" type="text" placeholder="auto-generated-if-empty" class="w-full bg-white" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div>
              ...
              <select id="prod-type" class="w-full bg-white cursor-pointer">
                <option value="Hijab">Single Hijab</option>
                <option value="Hamper Set">Hamper Set</option>
              </select>
            </div>
            <div>
              <label class="block text-xs font-semibold text-zinc-700 mb-1">Price (₹) *</label>
              <input id="prod-price" type="number" required value="899" class="w-full bg-white" />
            </div>
            <div>
              <label class="block text-xs font-semibold text-zinc-700 mb-1">Stock Units</label>
              <input id="prod-stock" type="number" required value="25" class="w-full bg-white" />
            </div>
          </div>

          <div class="px-6 py-3 border-t border-[#EAE3D2] bg-white flex justify-end gap-2 -mx-6 -mb-6 mt-4">
            ...
            <button type="submit" class="px-5 py-2 bg-[#5C1D24] text-white text-xs font-semibold rounded-xl hover:bg-[#47161c] cursor-pointer">Create Product</button>
          </div>
        </form>
      </div>
    </div>
  ...
  const modalHTML = `
    <div class="fixed inset-0 z-50 flex items-center justify-end modal-backdrop">
      <div class="bg-[#FAF7F2] w-full max-w-md h-full border-l border-[#EAE3D2] shadow-2xl flex flex-col justify-between overflow-hidden">
        <div>
          <div class="p-6 border-b border-[#EAE3D2] flex justify-between items-start bg-white">
            <div>
...
function openManualOrderModal() {
  const modalHTML = `
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop">
      <div class="bg-[#FAF7F2] w-full max-w-lg rounded-2xl border border-[#EAE3D2] shadow-2xl overflow-hidden">
        <div class="px-6 py-4 border-b border-[#EAE3D2] flex justify-between items-center bg-white">
          <h3 class="font-serif text-xl font-normal text-zinc-900">Log Manual WhatsApp / DM Order</h3>
          <button onclick="closeModal()" class="text-zinc-400 hover:text-zinc-700 cursor-pointer"><i data-lucide="x" class="w-5 h-5"></i></button>
        </div>
        <form onsubmit="handleCreateManualOrder(event)" class="p-6 space-y-3">
          <div>
...
function openAddReviewModal() {
  const modalHTML = `
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop">
      <div class="bg-[#FAF7F2] w-full max-w-md rounded-2xl border border-[#EAE3D2] shadow-2xl overflow-hidden">
        <div class="px-6 py-4 border-b border-[#EAE3D2] flex justify-between items-center bg-white">
          <h3 class="font-serif text-xl font-normal text-zinc-900">Add DM / Text Review</h3>
          <button onclick="closeModal()" class="text-zinc-400 hover:text-zinc-700 cursor-pointer"><i data-lucide="x" class="w-5 h-5"></i></button>
        </div>
        <form onsubmit="handleCreateReview(event)" class="p-6 space-y-3">
          <div>
...
// Temporary variant state during modal usage
let modalVariants = [];
...
function getProductFormHTML(isEdit = false, prod = {}) {
  ...
  return `
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop">
      <div class="bg-[#FAF7F2] w-full max-w-2xl rounded-2xl border border-[#EAE3D2] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        <div class="px-6 py-4 border-b border-[#EAE3D2] flex justify-between items-center bg-white">
          <h3 class="font-serif text-xl font-normal text-zinc-900">${isEdit ? 'Edit Product Details' : 'Add New Product or Hamper'}</h3>
          <button onclick="closeModal()" class="text-zinc-400 hover:text-zinc-700 cursor-pointer"><i data-lucide="x" class="w-5 h-5"></i></button>
        </div>

        <form onsubmit="${isEdit ? `handleUpdateProduct(event, '${prod.id}')` : 'handleCreateProduct(event)'}" class="p-6 overflow-y-auto space-y-5">
          <!-- Title & Slug -->
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-semibold text-zinc-700 mb-1">Title / Name *</label>
          ...
          <div class="p-4 rounded-xl border border-[#EAE3D2] bg-[#FAF7F2] space-y-3">
            <div class="flex justify-between items-center">
              <div>
                <label class="block text-xs font-bold text-zinc-800">Product Images (Upload Directly) *</label>
                <p class="text-[11px] text-zinc-500">Upload multiple photos directly from your device (no links required). Select multiple images at once.</p>
              </div>
              <span class="px-2.5 py-1 rounded-full bg-rose-100 text-rose-900 text-[10px] font-bold">0 images</span>
            </div>

            <div class="border-2 border-dashed border-[#D8D0C0] bg-white rounded-xl p-6 text-center space-y-2 hover:border-[#C5A059] transition-colors cursor-pointer">
              <div class="w-10 h-10 rounded-full bg-rose-50 text-[#5C1D24] flex items-center justify-center mx-auto">
                <i data-lucide="upload-cloud" class="w-5 h-5"></i>
              </div>
              <p class="text-xs font-semibold text-[#5C1D24]">Click to select images from device <span class="font-normal text-zinc-600">or drag &amp; drop files here</span></p>
              <p class="text-[10px] text-zinc-400">Supports JPEG, PNG, WEBP • Automatically compressed &amp; optimized</p>
            </div>

            <button type="button" class="text-[11px] text-[#5C1D24] hover:underline font-medium">+ Or paste external image URL (optional)</button>
          </div>

          <!-- Color & Style Variants -->
          <div class="p-4 rounded-xl border border-[#EAE3D2] bg-white space-y-3">
            ...
            <p class="text-[11px] text-zinc-500">Link specific uploaded photos to each shade for interactive swatching on the store.</p>
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-1.5">
                ...
                <button type="button" onclick="addPresetVariant('Royal Navy', '#1A2B4C')" class="px-2.5 py-1 rounded-full border border-zinc-200 bg-white text-[11px] font-medium text-zinc-700 flex items-center gap-1.5 hover:border-[#C5A059]">
                  <span class="w-2.5 h-2.5 rounded-full bg-[#1A2B4C]"></span> Royal Navy
                </button>
              </div>
            </div>
            ...
            <div class="p-3 bg-[#FAF7F2] rounded-xl border border-[#EAE3D2] grid grid-cols-1 sm:grid-cols-12 gap-2 items-center">
              <div class="sm:col-span-5">
                <label class="block text-[10px] font-semibold text-zinc-600 mb-0.5">Variant Name</label>
                <input id="variant-input-name" type="text" placeholder="e.g. Champagne Gold" class="w-full bg-white text-xs" />
              </div>
              <div class="sm:col-span-3">
                <label class="block text-[10px] font-semibold text-zinc-600 mb-0.5">Color Swatch</label>
                <div class="flex items-center gap-1">
                  <input id="variant-input-hex" type="color" value="#D4AF37" class="w-7 h-7 p-0 border-0 rounded cursor-pointer shrink-0" />
                  <input id="variant-input-hex-text" type="text" value="#D4AF37" class="w-full bg-white text-xs font-mono" />
                </div>
              </div>
...
function renderModalVariantList() {
  if (modalVariants.length === 0) {
    return `<p class="text-[11px] text-zinc-400 italic text-center py-2">No variants added yet. Add variants above to let customers pick color shades on the storefront.</p>`;
  }
  return modalVariants.map((v, i) => `
    <div class="p-2 rounded-lg bg-[#FAF7F2] border border-[#EAE3D2] flex justify-between items-center text-xs">
      <div class="flex items-center gap-2">
        ...
        <span class="text-[10px] text-zinc-400 font-mono">${v.hex}</span>
      </div>
...
function addCustomVariant() {
  ...
  const hexInput = document.getElementById('variant-input-hex-text') || document.getElementById('variant-input-hex');
  const name = nameInput.value.trim();
  const hex = hexInput.value.trim();

  if (name) {
    modalVariants.push({ name, hex: hex || '#D4AF37' });
    nameInput.value = '';
    updateModalVariantView();
  }
}
